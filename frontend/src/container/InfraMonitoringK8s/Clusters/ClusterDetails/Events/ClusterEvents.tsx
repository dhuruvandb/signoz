...
import './ClusterEvents.styles.scss';
...
import { getClustersEventsQueryPayload } from './constants';
...
export default function Events({
	...
	handleTimeChange,
	selectedInterval,
}: IClusterEventsProps): JSX.Element {
	const { currentQuery } = useQueryBuilder();

	const [formattedClusterEvents, setFormattedClusterEvents] = useState<
		EventDataType[]
	>([]);
	...
	const queryPayload = useMemo(() => {
		const basePayload = getClustersEventsQueryPayload(
			timeRange.startTime,
			timeRange.endTime,
			filters,
		);
	...
	const { data: eventsData, isLoading, isFetching, isError } = useQuery({
		queryKey: ['clusterEvents', timeRange.startTime, timeRange.endTime, filters],
	...
	useEffect(() => {
		if (eventsData?.payload?.data?.newResult?.data?.result) {
			...
			setFormattedClusterEvents(formattedData);
	...
	const handlePrev = (): void => {
		if (!formattedClusterEvents.length) return;

		setPage(page - 1);

		const firstEvent = formattedClusterEvents[0];
	...
	const handleNext = (): void => {
		...
		setPage(page + 1);
		const lastEvent = formattedClusterEvents[formattedClusterEvents.length - 1];
	...
	return (
		<div className="cluster-events-container">
			<div className="cluster-events-header">
			...
			{!isLoading && !isError && formattedClusterEvents.length === 0 && (
				<NoEventsContainer />
			)}
			...
			{!isLoading && !isError && formattedClusterEvents.length > 0 && (
				<div className="cluster-events-list-container">
					<div className="cluster-events-list-card">
						<Table<EventDataType>
							...
							dataSource={formattedClusterEvents}
							pagination={false}
			...
			{!isError && formattedClusterEvents.length > 0 && (
				<div className="cluster-events-footer">
					<Button
						className="cluster-events-footer-button periscope-btn ghost"
						type="link"
						onClick={handlePrev}
