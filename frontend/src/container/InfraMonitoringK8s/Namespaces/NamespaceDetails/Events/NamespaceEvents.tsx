...
export default function Events({
	...
	const [formattedNamespaceEvents, setFormattedNamespaceEvents] = useState<
		EventDataType[]
	>([]);
	...
	useEffect(() => {
		if (eventsData?.payload?.data?.newResult?.data?.result) {
			...
			setFormattedNamespaceEvents(formattedData);
	...
	const handlePrev = (): void => {
		if (!formattedNamespaceEvents.length) return;

		setPage(page - 1);

		const firstEvent = formattedNamespaceEvents[0];
	...
	const handleNext = (): void => {
		...
		setPage(page + 1);
		const lastEvent =
			formattedNamespaceEvents[formattedNamespaceEvents.length - 1];
